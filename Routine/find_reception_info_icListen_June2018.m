...
now_day = day(1);
while true
    cd("/Volumes/ACO_RAP_3/RAP/June2018Cruise/wav_data/icListen/"+string(now_day)+"_June")
%     /Volumes/ACO_RAP_3/June2018Cruise/wav_data/icListen/22_June
    while now_hour <= 23
        if now_hour < 10
            fname_d = "SBW1391_201806"+string(now_day)+"_0"+string(now_hour);
        else
            fname_d =  "SBW1391_201806"+string(now_day)+"_"+string(now_hour);
        end
        d = dir(fname_d+"*");
...
while true
    ...
    while true
        ...
        cd("/Volumes/ACO_RAP_3/RAP/June2018Cruise/wav_data/icListen/"+string(now_day)+"_June")    % EDIT
        %Load Audio Data
        wav_name = au_fname(counter,:)
...
%% Save hourly file
% 3. Rx File Directory
cd /Volumes/ACO_RAP_3/RAP/June2018Cruise/Tx_Rx_Output/rx_file/icListen/June2018/original_depth % EDIT
% Save Variables
...
icListen_depth = -4729.92+2.32 +1.75;          % original depth ellipsoid height+ 1.75 m higher than HEM
 ...
 % 2. Tx File Directory
cd("/Volumes/ACO_RAP_3/RAP/June2018Cruise/Tx_Rx_Output/tx_file/all")  %% EDIT 
...
% load 2 files
    if now_day == 7 & now_hour == 2
        fname = "tx_data_2018_06_07_02";
    else
       for p = 1:2
            if p ==1 & now_hour == 0
                fname_d = "tx_data_2018_06_"+string(now_day-1)+"_"+string(23);
            elseif p==1 & now_hour == 10
                fname_d = "tx_data_2018_06_"+string(now_day)+"_0"+string(9);
            elseif now_hour < 10
                fname_d = "tx_data_2018_06_"+string(now_day)+"_0"+string(now_hour-2+p);
            else
                fname_d = "tx_data_2018_06_"+string(now_day)+"_"+string(now_hour-2+p);
            end
