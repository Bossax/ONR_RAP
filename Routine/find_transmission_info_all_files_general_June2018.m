...
close all
%%%% For June 2018
cd /Volumes/ACO_RAP_2/RAP/June2018Cruise/Scarlett_Data
d=dir('LFM_*.mat');     
mod_date = [];
mod_hr = [];
...
for i = 1:length(d)
    s = d(i).name;
    mod_date(end+1) = str2num(s(7:8))-47;
    mod_hr(end+1) = str2num(s(9:10));
end
%%
% extract pos_mv files date hr
cd /Volumes/ACO_RAP_2/RAP/June2018Cruise/Tx_Rx_Output/posmv/all
d_pos=dir('pos*.mat');          
...
for i = 1:length(d_pos)
    ...
    pos_date(end+1) = str2num(pos_name(end-7:end-6));
    pos_hr(end+1) = str2num(pos_name(end-5:end-4));
end
...
for k = 1:length(group)     
    ...
    for i=start_c:end_c
        ...
        peak_num=max(V);
        A=find(abs(V)>=peak_num/2);          %Find voltage spike
    ...
    for i=1:length(tx_time)
        ...
        tx_altitude_err =   horzcat(tx_altitude_err,altitude_err(B));
        tx_lat_err =  horzcat(tx_lat_err,lat_err((B)));
        tx_lon_err = horzcat(tx_lon_err,lon_err((B)));
        tx_roll_err = horzcat(tx_roll_err,roll_err((B)));
        tx_pitch_err = horzcat(tx_pitch_err,pitch_err((B)));
        tx_heading_err = horzcat(tx_heading_err ,heading_err((B)));
        tx_t_err =  horzcat(tx_t_err,t_err((B)));
    ...
    save_fname = sprintf("tx_data_2018_06_%.0f_%.0f",pos_date(pos_file_index(1)),pos_hr(pos_file_index(1)));
    if pos_hr(pos_file_index(1)) <10
       save_fname = sprintf("tx_data_2018_06_%.0f_0%.0f",pos_date(pos_file_index(1)),pos_hr(pos_file_index(1))) ;
    end
    save_fname
