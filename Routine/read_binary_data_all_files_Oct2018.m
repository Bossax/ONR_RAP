...
% SET-UP: Edit the date of the Sunday of the week in line ~153-156
% save the output files in .mat extension with associated labels

clearvars
close all
cd /Volumes/ACO_RAP_2/RAP/Oct2018Cruise/POS-MV/after                                 % Navigate to binary file directory

directory=dir('pos-bin*');

for j =1:length(directory)
fname = directory(j).name;

label = string(fname(end-9:end));
save_fname = "posmv_"+label;
fprintf(save_fname +"\n.\n.\n")

num_bytes=0;
i=1;
cd /Volumes/ACO_RAP_2/RAP/Oct2018Cruise/POS-MV/after
fid=fopen(fname,'r');
...
t=datenum(2018,10,sunday)+t1./(3600*24);
t_err=datenum(2018,10,sunday)+t1_1./(3600*24);
