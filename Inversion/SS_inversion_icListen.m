...
[lon_u,~,~] = m_fdist(icListen_lon,icListen_lat,90,L);
[~,lat_l,~] = m_fdist(icListen_lon,icListen_lat,180,L);
[lon_l,~,~] = m_fdist(icListen_lon,icListen_lat,270,L);
n_wall = grid_num+1;
lon_l = lon_l-360;
lon_u = lon_u-360;
...
% Function = 0.4762*range;
% Function = 0.3961*range;
Function = 0.4082*range;
...
for ii = 1:length(y_cen)
    for jj = 1:length(x_cen)
            sur_dist(kk) = dist([x_cen(jj) icListen_lon],[y_cen(ii) icListen_lat]);
            azmth(kk) = azimuth(icListen_lat,icListen_lon,y_cen(ii),x_cen(jj));
            kk = kk+1;
    end
end      
...
xlabel('Range (km)')
ylabel('Resolution')
title({'Resolution versus Range: Depth-averaged solution',' (Covariance Length = 8.6 km)'})
set(gca,'fontsize',14)
line([0 50],[0 0],'color','k')
ylim([0 1])
