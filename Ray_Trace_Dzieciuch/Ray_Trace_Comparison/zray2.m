% main function
function zry=zray2( theta0, srcdep, rgsv, reltol)
% Inputs
% 1.launch angles, both above and below the horizontal line (angle relative to the vertical)
% 2. source depth
% 3. incremental range vector,
% 4. acceptable error
...
nbblimit=20;    % bottom bouncing limit 
nsblimit=1000;  % surface bouncing limit
swrgsv=0;

% rays to be launched
nray=length(theta0);

% set up surface range
rgsv=rgsv(:);
rstart=rgsv(1);
rend=rgsv(end);
...
for ir=1:nray
%     ir
    % write out angle being launched (grazing angle)
%     fprintf('theta0: %7.3f\n', theta0(ir));
    ...
    % time
    % depth
    % ray parameter (p)/ ray angle
    % weighting
    % arc length
    X0(idt)=0;
    X0(idz)=srcdep;
    X0(idp)= - sin(pi*theta0(ir)/180)./c0;    % ray parameter/ angle
    X0(idw) = 0;
    X0(ids)=0;
    ...
    options.events=@zrayrflct;      % reflection event (ray hits interface or turns)
    
    opts=odeset( 'reltol', options.reltol);                % Relative error tolerance 
    opts=odeset( opts, 'abstol', options.abstol);          %  Absolute error tolerance
    opts=odeset( opts, 'initialstep', options.initialstep); % Suggested initial step size
    opts=odeset( opts, 'events', options.events);           % set up reflection event
    
    rnow = rstart; Xnow=X0;
    rx = rnow; X = Xnow;
    rev=[];
    % loop over horizontal distance
    while (rnow < rend)
%         warning off
        rgo=[rnow, rend];
        ...
        % integrate ODE
        % returns
        % rint = increased horizontal ranges
        % Xint = increased ray state parameters
        [rint,Xint,rv,Xv,Ip]=ode45( @zraydot2, rgo, Xnow, opts);
        %[rint,Xint,rv,Xv,Ip]=ode45x( rgo, Xnow, options);
        warning on
        
        % rv, Xv = parameters when the event is triggered
        % Ip = event trigger indicator 
        
        % update range and state vector
...
%             disp('lower turning ')
        ...
        if (nsb>=nsblimit) 
            disp('Too many suface bounces')
            break;
        end
        %Check if ray is too steep
        [cnow, ~]=zrayc( Xnow(idz), rnow);
        anow=asin(-cnow*Xnow(idp)); % angle
        if (abs(anow)>(pi*thetasteep)/180)
            disp('ray too steep')
            break; 
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     plot( rx(2:end), diff(-X(:,idw))./diff(rx),'k');
%     xlabel('Range')
%     ylabel('Ray Weighting')
%     hold on
...
while ~done
    ...
    while true
        hA = h*A;
        hB = h*B;
        f(:,2) = zraydot2(t+hA(1),y+f*hB(:,1));
        f(:,3) = zraydot2(t+hA(2),y+f*hB(:,2));
        f(:,4) = zraydot2(t+hA(3),y+f*hB(:,3));
        f(:,5) = zraydot2(t+hA(4),y+f*hB(:,4));
        f(:,6) = zraydot2(t+hA(5),y+f*hB(:,5));
