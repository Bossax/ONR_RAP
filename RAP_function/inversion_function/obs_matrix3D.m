function [obs_ray,total_pixel_num,theta_l,theta_r,intd_G,z_cross,intd_G_all,z,SS_priori,r_layer]=obs_matrix(tx_lat,tx_lon,tx_altitude,ACO_lat,ACO_lon,ACO_depth,x_tick,y_tick,mode)
%  oberservation_matrix construction
% 1. Find the surface distance of an acoustic ray travels within one pixel
% 2. match the surface distance with the ray tarcing calcluation to divide the ray path into pixel-wise arc lengths
...
intd_G = zeros(length(total_pixel_num),1);
r_layer = zeros(length(total_pixel_num),1);
for iii = 1:length(total_pixel_num)    
