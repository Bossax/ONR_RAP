...
load LFM_pulse_ideal_icListen.mat
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
threshold= 9;    % 9 is the original number
    ...
    for i=1:length(demod_separation)+1
            ...
            % if peaks are detected, output all demod_pos, demod_max,demod_snr
                 ...
                 if length(A) > 1
                     ...
                     if t_sep_d < 0.0015
                        ...
                        t_sep =  vertcat(t_sep,(locs(A(2))-locs(A(1)))/fs);
                     else
                    ...
                    if A(1) == length(locs)
                        ...
                        t_sep_d = (locs(A(1)+1)-locs(A(1)))/fs;
                        if t_sep_d < 0.0015
                             ...
                             t_sep =  vertcat(t_sep,(locs(A(1)+1)-locs(A(1)))/fs);
                        else
                             demod_snr_pks= vertcat(demod_snr_pks,10*log10([pks(A(1)) ; pks(A(1))]./median(demod)));
                             demod_max = vertcat(demod_max,[pks(A(1)); pks(A(1))]);
        ...
        elseif i==length(demod_separation)+1
            ...
            if length(A) > 1
                ...
                t_sep_d = (locs(A(2))-locs(A(1)))/fs; 
                if t_sep_d < 0.0015
                    demod_snr_pks= vertcat(demod_snr_pks,10*log10([pks(A(1)) ;pks(A(2))]/median(demod)));
                    demod_max = vertcat(demod_max,pks(A(1:2)));
