...
cd /Volumes/LaCie_RAP_Backup/RAP/June2018Cruise/Tx_Rx_Output/rx_files

load('rx_data_before_depart_icListen.mat')       % Edit
...
cd /Volumes/LaCie_RAP_Backup/RAP/June2018Cruise/Tx_Rx_Output/Path/before_depart
d_tx=dir('tx_data_*.mat');
lat = [];
lon = [];
t_tx = [];

% Cut out non-usable data points 
...
cd /Volumes/LaCie_RAP_Backup/RAP/June2018Cruise/wav_data/icListen/before_depart % Edit
hyd = dir('SBW*');
addpath('/Volumes/LaCie_RAP_Backup/RAP/June2018Cruise/Script/Signal_Arrival')

for jj = 1:1:length(hyd)
    fname = hyd(jj).name
    
    [y,t_date] = icListen_audio_prep(fname);
...
% Axis Ticks
yvalue = [start_tx_num:1:tx_num_max-1 tx_num_max];                    %Edit
y_label = yvalue;                  
