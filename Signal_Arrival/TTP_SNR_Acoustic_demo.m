...
for i =23:2:length(au_fname)
     now_day = au_fname(i,5:6);
     now_hour = au_fname(i,8:9);
     now_min = au_fname(i,11:12);
     cd("/Volumes/ACO_RAP_2/RAP/Oct2018Cruise/wav_data/HEM/"+string(now_day)+"_Oct")
      %Load Audio Data
      mat_name = au_fname(i,:);
    ...
    %%% Save data
     ...
     ttp_snr_sig_data.act_arrival = act_arrival;
     ttp_snr_sig_data.env_pk = env_pk;
     ttp_snr_sig_data.envpk_pos = envpk_pos;
     sname = ['ttp_snr_sig_data' '_2018_10_' now_day '_' now_hour];
     cd '/Volumes/ACO_RAP_2/RAP/Oct2018Cruise/Tx_Rx_Output/ttp_snr_plot'
