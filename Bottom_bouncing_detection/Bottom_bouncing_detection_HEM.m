...
cd /Volumes/ACO_RAP_2/RAP/Oct2018Cruise/Tx_Rx_Output/tx_file/all/2
...
scale_signal = 0.3/10^7;               % Edit
scale_snr = .5/10^9;               % Edit % ideal pulse = 10^9 / sample pulse  10^15
...
for j = 1:disp_round
    ...
    hold on 
    plot(duration,SNR,'Color',[0 0.6 0.8])                  
    grid minor
    axis tight 
    title('Complex Envelopes and Raw Signal: HEM')
    xlabel('msec')
    set(gca,'fontsize',15)
    % peak locations
    hold on
